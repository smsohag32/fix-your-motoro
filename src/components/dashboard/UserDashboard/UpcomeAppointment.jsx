...
const UpcomeAppointment = () => {
    ...
    const { data: workOrder, isLoading: wLoading } = useQuery({
        ...
        enabled: !loading,
        queryFn: async () => {
          const res = await axios.get(`https://fya-backend.vercel.app/api/v1/auth/user-order/${user?.email}`);
          return res.data;
        },
      });
    ...
    return (
        ...
        <div className="my-5">
            <h1 className="md:text-2xl text-xl font-semibold opacity-75">Up Comming Appointment</h1>
        </div>
