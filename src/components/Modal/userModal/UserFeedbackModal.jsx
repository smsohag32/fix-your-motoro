...
const UserFeedbackModal = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <Modal isOpen={isOpen} setIsOpen={setIsOpen} title="Give Your Valuable Feedback">
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <input type="hidden" id="rating" name="rating" {...register("rating")} />

                <div className="flex justify-center gap-4 mt-6">
                    <button onClick={() => onCancel()} type="button" className="delete-btn rounded-lg w-full md:w-auto">
                        Cancel
                    </button>
                    <button type="submit" className="w-full md:w-auto px-3 py-2 text-center transition-all duration-500 font-semibold bg-sky-600 text-white hover:bg-sky-800 rounded-lg">
                        Submit
                    </button>
                </div>
                <Toaster />
